...
1. Dev version line pre-move: dispatch dev-version-bump.yml (default branch)
   with intended-version=2.53.0, mode=pre-move. Merge the PR it opens
   (expected: dev 2.53.0 -> 2.54.0). release.yml refuses publish until dev
   outranks 2.53.0, so this merge must land first.
2. Confirm dev head post-merge; green exact-head hosted CI on that head is
   ...
   re-evaluate.
3. Main promotion in a fresh scratch worktree (same recipe as wp4, branch off
   origin/main, branch name codex/release-2530-main, package.json version
   2.53.0, tree = audited dev product tree 981b53e7d0):
...
4. Publish: dispatch release.yml on branch main with version=2.53.0,
   tag=latest, expected-sha=<main merge commit>, dry-run=false.
